...
def timesPosition(arr:Array[Int]):Array[Int] = {
  arr.zipWithIndex.map((x, y) => x * y).toArray 
}
...
def matchingLetters(wordA:String, wordB:String):List[(Int, Int)] = {
  (for 
    x <- wordA.zipWithIndex
    y <- wordB.zipWithIndex if (x._1 == y._1)
  yield(x._2, y._2)).toList
}
...
def roman(i:Int):String =
  ...
  val convertion = List("M" -> 1000, "CM" -> 900, "D" -> 500, "CD" -> 400, "C" -> 100, 
    "XC" -> 90, "L" -> 50, "XL" -> 40, "X" -> 10, "IX" -> 9, "V" -> 5, "IV" -> 4, "I" -> 1)
 
  def recursedRomans(i: Int, numerals: String): String =
    i match 
      case _ if i >= 1000 => recursedRomans(i - 1000, numerals + "M")
      case _ if i >= 900 => recursedRomans(i - 900, numerals + "CM")
      case _ if i >= 500 => recursedRomans(i - 500, numerals + "D")
      case _ if i >= 400 => recursedRomans(i - 400, numerals + "CD")
      case _ if i >= 100 => recursedRomans(i - 100, numerals + "C")
      case _ if i >= 90 => recursedRomans(i - 90, numerals + "XC")
      case _ if i >= 50 => recursedRomans(i - 50, numerals + "L")
      case _ if i >= 40 => recursedRomans(i - 40, numerals + "XL")
      case _ if i >= 10 => recursedRomans(i - 10, numerals + "X")
      case _ if i >= 9 => recursedRomans(i - 9, numerals + "IX")
      case _ if i >= 5 => recursedRomans(i - 5, numerals + "V")
      case _ if i >= 4 => recursedRomans(i - 4, numerals + "IV")
      case _ if i >= 1 => recursedRomans(i - 1, numerals + "I")
      case _ => numerals   
