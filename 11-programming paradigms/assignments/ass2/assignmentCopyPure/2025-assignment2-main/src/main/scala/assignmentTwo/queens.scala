...
extension (grid:Grid) {
    def colours:Set[Colour] = 
        (for row <- grid; col <- row yield col).toSet
    ...
    def allSquares:Seq[Location] = 
        for y <- grid.indices; x <- grid(y).indices yield (x, y)

    def squares(colour:Colour):Seq[Location] =
        for sq <- allSquares if grid(sq).contains(colour) yield sq    

    def neighbours(loc:Location):Seq[Location] =
        for d <- Direction.values.toSeq; p = loc + d if grid.allSquares.contains(p) yield p

    def allPossibilities:PossibilityMap = 
        (for y <- grid.indices; x <- grid(y).indices yield (x, y) -> Contents.values.toSet).toMap
}
...
extension (map:PossibilityMap) {
    ...
    def setQueen(grid:Grid, loc:Location):PossibilityMap = 
        val thereforBlank:Seq[Location] =
            grid.column(loc._1).filter(_ != loc) ++
            grid.row(loc._2).filter(_ != loc) ++
            grid(loc).map(grid.squares).getOrElse(Seq()) ++
            grid.neighbours(loc)
    ...
    def rowFails(grid:Grid):Boolean = 
        grid.indices.exists { y =>
            val row = grid.row(y)
            val statuses = row.map(map(_))
            statuses.count(_ == Set(Contents.Queen)) > 1 || statuses.forall(_ == Set(Contents.Blank))
        }

    def columnFails(grid:Grid):Boolean = 
        grid(0).indices.exists { x =>
            val col = grid.column(x)
            val statuses = col.map(map(_))
            statuses.count(_ == Set(Contents.Queen)) > 1 || statuses.forall(_ == Set(Contents.Blank))
        }

    def colourFails(grid:Grid):Boolean = 
        grid.colours.exists { colour =>
            val locations = grid.squares(colour)
            val statuses = locations.map(map(_))
            statuses.count(_ == Set(Contents.Queen)) > 1 || statuses.forall(_ == Set(Contents.Blank))
        }

    def extraRuleFails(grid:Grid):Boolean = false

    def invalidFor(grid:Grid):Boolean = 
        hasTwoQueensTouching(grid) || rowFails(grid) || columnFails(grid) || colourFails(grid) || extraRuleFails(grid)

    def makeAstep(grid:Grid):PossibilityMap =
        val undecided = map.filter((_, s) => s.size == 2).keys

        val forcedQueen = undecided.find { loc =>
            val testMap = map.setBlank(grid, loc)
            testMap.invalidFor(grid)
        }

        forcedQueen match
            case Some(loc) => return map.setQueen(grid, loc)
            case None =>
                val forcedBlank = undecided.find { loc =>
                    val testMap = map.setQueen(grid, loc)
                    testMap.invalidFor(grid)
                }
                forcedBlank match
                    case Some(loc) => return map.setBlank(grid, loc)
                    case None => map // no step made
} 
